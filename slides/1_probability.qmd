...
A random variable $X$ is a function that maps each outcome $\omega$ to an event expressed as a number.
...
[Can also think of $X$ as a random process that produces numbers as outcomes, but this way distinguishes the random process itself from the researcher's representation of it in numbers.]{style="font-size: 50%; line-height: 10%"}
...
This gives us a number to describe $X$ -- **not** a new random variable. 
