...
where $\mathbb{X}$ is an $n \times K$ matrix of predictors and $\mathbf{Y}$ is an $n \times 1$ column vector (the dependent variable). 

We'll discuss more.

. . . 

For we use `lm()` or `estimatr::lm_robust()` in `R`. 
...
## What a BLP can be good for  {.smaller}

Say we are predicting infant mortality $Y$ using water quality and access to electricity [(Aronow and Miller)]{.gray}.

. . . 

<br>

Why estimate this BLP?

- It might provide a useful *description* of relationships, i.e. a summary of how these things vary across countries
- It might be a useful *predictive tool* (e.g. likely infant mortality in regions where we have data on water quality and electrification)
- It might inform us about (causal) **effect**, i.e. likely change in infant mortaility if we changed water quality or access to electricity
...
- we can specify a BLP that is a very flexible function of a predictor $X$ with polynomials, binning, splines, - more flexibility $\rightarrow$ better fit (lower mean squared residuals) in the sample
...
bb$treatment <- ifelse(bb$treat_deshawn == 1, "2. Deshawn", "1. Jake")
lm(reply_atall ~ treatment, data = bb) |> coef()
```
...
- think about predictions for examples, e.g. "email from Jake" vs "email from Deshawn"
